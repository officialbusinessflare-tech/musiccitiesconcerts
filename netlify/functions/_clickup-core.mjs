 ...
 *
 * Shared ClickUp fetching + normalization logic. Imported by:
 *   - scripts/fetch-clickup.js (local CLI)
 *   - netlify/functions/nightly-rebuild.mjs (scheduled function)
 *
 * Plain Node ESM. Keep parseTaskName/parseVenueLocation in lock-step with
 * src/lib/parse-name.ts — same algorithm, two implementations.
 */
...
// Minimum candidate band-name length. Sub-3-char fragments (e.g. "A", "Co")
// create huge false-positive matches against the watch-list via includes().
 ...
 * (e.g. `&amp;` instead of `&`). Without this, splits on `&` leave `amp;`
 * fragments behind that become bogus band names.
 */
...
function parseBandsFromHeadline(headlineSegment, isRelease) {
        ...
        return cleaned
          ...
          .map((s) => s.trim())
          // Drop empties AND sub-3-char fragments that produce huge
